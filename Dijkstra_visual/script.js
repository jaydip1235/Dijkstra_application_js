onload = function() {
    ...
    const cities = ['Kolkata', 'Delhi', 'Gujarat', 'Kanpur', 'Patna', 'Jammu', 'Roorkee', 'Bangalore', 'Gangtok', 'Lucknow'];
    ...
    function createData() {
        ...
        for (let i = 1; i <= V / 2;) {
            ...
            if (n1 !== n2) {
                ...
                // If works == 0, add bus as well as plane between vertices
                // If works == 1, only add plane between them
                if (works <= 1) {
                    if (works === 0 && i < V / 4) {
                        // Adding a bus
                        edges.push({
    ...
    function solveData() {
        ...
        let new_edges = [];
        if (plane !== 0) { //if a plane can be taken
